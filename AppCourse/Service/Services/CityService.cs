...
namespace Service.Services
{
    public class CityService : ICityService
    {
        ...
        public async Task<PaginationResponse<CityDto>> GetPaginateDataAsync(int page, int take)
        {
            var cities = await _cityRepo.GetAllAsync();
            int totalPage = (int)Math.Ceiling((decimal)cities.Count() / take);

            var mappedDatas = _mapper.Map<IEnumerable<CityDto>>(await _cityRepo.GetPaginateDataAsync(page, take));
